...
const alertTypeButtons = document.getElementById('alertTypeButtons');
...
alertTypeButtons.addEventListener('click', (event) => {
  const btn = event.target.closest('.alert-type-btn');
  if (!btn) return;
  setAlertType(btn.dataset.type);
});
